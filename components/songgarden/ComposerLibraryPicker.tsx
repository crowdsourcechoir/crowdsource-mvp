...
const GROUP_LABEL_CLASS =
  "px-3 pb-1 pt-2 text-[9px] font-semibold uppercase tracking-[0.18em] text-gray-500";

function rowClass(active: boolean, muted = false): string {
  const base =
    "flex w-full min-w-0 items-center gap-2 rounded-lg text-left text-xs transition-colors focus:outline-none focus-visible:ring-1 focus-visible:ring-[var(--csc-accent)]/60";
    ...
    open && placement && mounted
      ? createPortal(
          <div
            ...
            className="fixed z-[200] overflow-hidden rounded-lg border border-white/15 bg-black shadow-xl"
            style={{
              top: placement.top,
              left: placement.left,
          ...
          >
            <div className="h-full overflow-y-auto overscroll-contain py-1 text-xs" style={{ maxHeight: placement.maxHeight }}>
              <button
                type="button"
                role="option"
              ...
              >
                <span className="min-w-0 truncate">Master — all sounds</span>
              </button>
                ...
                return (
                  <div key={garden.id}>
                    <div className="flex items-stretch gap-0.5">
                      ...
                      >
                        <span className="min-w-0 truncate">{garden.title}</span>
                      </button>
                      {garden.blooms.length > 0 ? (
                        <button
                          ...
                          className="shrink-0 rounded-lg px-2 text-gray-500 transition-colors hover:bg-[var(--csc-accent)]/10 hover:text-[var(--csc-accent)] focus:outline-none focus-visible:ring-1 focus-visible:ring-[var(--csc-accent)]/60"
                        >
                          <svg
                            viewBox="0 0 12 12"
                          ...
                          return (
                            ...
                            >
                              <span
                                ...
                                aria-hidden
                              />
                              <span className="min-w-0 truncate">{bloom.title}</span>
                            </button>
                          );
                        })
                      : null}
  ...
  return (
    <div ref={rootRef} className="relative min-w-0">
      <button
        ...
        title={label}
        onClick={() => setOpen((v) => !v)}
        className="inline-flex max-w-full items-center gap-2 rounded-lg border border-white/15 bg-black px-3 py-1.5 text-xs font-medium text-gray-200 transition-colors hover:border-[var(--csc-accent)] hover:text-white disabled:opacity-50 sm:max-w-[16rem]"
      >
        <span className="shrink-0 text-gray-500">Library</span>
        <span className="min-w-0 truncate text-white">{loading ? "…" : label}</span>
