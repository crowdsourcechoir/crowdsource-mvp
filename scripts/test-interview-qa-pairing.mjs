...
async function main() {
  ...
  const answers = pairInterviewAnswers([
    ...
    { id: "turn-joel", role: "user", content: "Joel", turn_index: 1, created_at: "2026-01-01T00:00:01Z" },
  ...
  assert.equal(answers[0].turnId, "turn-joel");
  assert.equal(answers[1].turnId, null);
