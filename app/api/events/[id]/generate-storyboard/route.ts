...
async function generateOneFrame(opts: {
  ...
  const { eventId, vibePrompt, frameIndex, frameCount, mode, placeUris, siblingSceneUrls } = opts;

  const { referenceImages, placeTags, siblingTags } = mergeStoryboardReferences({
    placeUris,
    siblingSceneUrls: mode === "landmarks" ? [] : siblingSceneUrls,
    frameIndex,
    frameCount,
  });

  const runwayImageUrl = await generateImageFromText({
    promptText: buildStoryboardImagePrompt(vibePrompt, frameIndex, frameCount, mode, {
      placeTags,
      siblingTags: mode === "landmarks" ? [] : siblingTags,
    }),
    model: "gen4_image",
    ratio: "1920:1080",
  ...
  const runwayVideoUrl = await generateVideoFromImage({
    ...
    promptText: buildStoryboardMotionPrompt(vibePrompt, frameIndex, frameCount, mode),
    model: "gen4_turbo",
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const mode = parseBoardMode(body.boardMode);
    ...
    return NextResponse.json(
      {
        error:
          mode === "landmarks"
            ? "Describe the vibe (city, light, community) so Runway can enhance each landmark."
            : "Describe the vibe (place, mood, community) so Runway can invent the world.",
      },
      { status: 400 }
    );
  }

  if (mode === "landmarks" && placeUris.length === 0 && body.append) {
    return NextResponse.json(
      {
        error:
          "Landmarks mode needs one place reference photo per frame. Add a ref, then Add AI frame.",
      },
      { status: 400 }
    );
  }
  ...
  if (singleFrameIndex != null || body.append) {
    const siblingSceneUrls = Array.isArray(body.siblingSceneUrls) ? body.siblingSceneUrls : [];
    const frameIndex =
      singleFrameIndex != null ? singleFrameIndex : Math.max(0, siblingSceneUrls.length);
  ...
  return NextResponse.json({ frames, boardMode: mode });
}
