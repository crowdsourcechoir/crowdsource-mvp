...
| `gmail_thread_id` | text, nullable | **Added in `sales-platform-add-gmail.sql`.** Gmail thread for the outreach conversation after first successful send. |
| `last_outbound_at` / `last_inbound_at` / `next_follow_up_at` | timestamptz, nullable | **Added in `sales-platform-add-gmail.sql`.** Touch timestamps for reply detection and nudge scheduling. |
| `import_metadata` | jsonb, nullable | Same purpose as `organizations.import_metadata` |
| `created_at` / `updated_at` | timestamptz | |

### `gmail_connections` / `outreach_feedback`
Added in `supabase/sales-platform-add-gmail.sql`. Single-operator OAuth row (`owner_key = 'default'`) with encrypted refresh token. Resume sending prefers `sends_enabled` when that column exists (`sales-platform-add-gmail-send-safety.sql`, optional) and otherwise stores a `csc:gmail-sends-enabled` marker on `scopes` so reconnect works if the SQL editor times out. Feedback rows capture approve-with-edits / reject pairs for draft few-shots. See `ai-workflow.md` §10.5.
