...
| `5, 15, 25, 35, 45, 55` past 9; also `20` past 10–12 | every day | `/api/sales/cron/pipeline` | Runs the full 10-stage pipeline on a time-boxed batch of pending organizations (`lib/sales/pipeline/run-pipeline-batch.ts`). **Six invocations through 9 UTC plus three mid-morning follow-ups**, each ending with `ensureDigestTarget` so processing keeps going until 10 leads score 70+. |
| `10` / `40` past 10–13 | every day | `/api/sales/cron/digest` | Sends only once ≥ `SALES_DIGEST_TARGET_COUNT` (default **10**) new/recovered queue items score ≥ `SALES_DIGEST_MIN_SCORE` (default **70**). Each tick tops up discovery, pipeline, and near-miss reprocess (45–69 deepen+rescore) within its budget; otherwise returns `deferred` and the next tick continues. |
