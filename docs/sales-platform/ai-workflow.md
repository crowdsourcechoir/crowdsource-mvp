...
- **Event-date extraction:** the LLM is instructed to emit `event_date` only when a real calendar date/range is present (not an event name/year alone). A deterministic backstop (`lib/sales/research/extractEventDates.ts`) also scans each fetched page for month/day/year patterns near convention/conference wording (e.g. `Dates: March 4-6, 2027`) so publicly posted dates aren't lost when the model tags the event title instead.
- **Near-miss / date-gap deepen pass:** after the first score, a second `research` agent_runs pass (`lib/sales/pipeline/stages/deepenResearch.ts`) uses the same Tavily/Serper keys as discovery when either (a) the total is in **45–69** (full deepen: dates + attendance + leadership + scorer gaps), or (b) the total is already **≥70** but findings still lack a calendar event date (dates-focused deepen). Both paths extract more findings, optionally refresh contacts, and **re-score once**. Date/schedule queries are first-class; scorer `missingInformation` gaps are interleaved into the query list rather than appended-then-dropped by the query cap.
...
- **Generic mailboxes are never treated as a "named person" by the pipeline:** an `info@org.com`-style address or a role string with no attached human name is excluded from AI contact discovery. **Operator-added general inboxes are sendable:** `info@`, `events@`, and similar shared mailboxes can be added, selected, drafted (`Hi there,`), and sent without Hunter `verified_deliverable` (catch-all domains usually come back `risky` / `accept_all`). Hunter still blocks them if the verifier says `invalid`. Named people still need a Hunter-verified deliverable address.
...
- **Verified-contact gate (queue entry):** an opportunity does **not** reach the human review queue (stage 10) unless `pickBestContact`'s result clears `lib/sales/dedupe.ts#isSendableContact` — a named person with Hunter `verified_deliverable`, **or** a general inbox like `info@` / `events@` that is not known-invalid. Scoring and brief generation (stages 6-7) still run regardless. `risky` and `unverified` do not clear the bar for named people.
...
- **Primary mechanism:** when Gmail is connected (`gmail_connections`, OAuth via `/api/sales/gmail/connect`) **and sending is resumed** (`sends_enabled` on that row, or a `csc:gmail-sends-enabled` marker on `scopes` if that column is missing, or `SALES_GMAIL_SENDS_ENABLED=true`), the decision route (`app/api/sales/queue/[itemId]/decision/route.ts`) claims the open draft then sends via the Gmail API (`lib/sales/gmail/send.ts`) (fail-closed: a send failure reverts the draft and leaves the item pending). A second initial send to the same contact is blocked unless the draft was reminted after the last send. Writes `outreach_activities` (`approved`, `sent`) with `gmail_message_id` / `gmail_thread_id`, and sets `opportunities.last_outbound_at`, `gmail_thread_id`, and `next_follow_up_at` (+7 days).
- **Reconnect:** Connect Gmail on `/admin/sales`, then **Resume sending**. Reconnect alone stays paused. Pause sending disconnects outbound without dropping OAuth. `SALES_GMAIL_SENDS_ENABLED=false` is an emergency kill that wins over the UI toggle.
