...
- **Voice is steered by real few-shot examples, not abstract adjectives** (`lib/sales/pipeline/stages/draft.ts#VOICE_REFERENCE_EMAILS`): the system prompt includes two real emails Joel has actually sent, verbatim, as the model's only voice reference — this steers tone far more reliably than instructions like "sound less salesy." Everything in those emails that's structurally fixed (greeting, self-intro, "I've attached a one-page overview...", sign-off, signature block/press quote) lives in `body_template` as literal text, not AI-authored — the model only ever fills `subject`, `openingReason` (the one-sentence bridge to the specific opportunity), and `fitReason` (the paragraph explaining why it fits). If the real voice drifts (new signature, new phrasing habits), update the two reference emails rather than hand-tuning the prose rules further — the examples do most of the work. QA (stage 9) is told about this deliberately: it must not flag the standard "I hope you're doing well!" greeting as fabricated familiarity, nor flag vivid self-description of Crowdsource Choir's own format as "generic company description" — both are sanctioned, expected voice, not violations.
...
- **Attachments:** `mailto:` links cannot carry file attachments in any browser/OS combination for security reasons, so the one-sheet/overview mentioned in the email body (`{{cta}}`-adjacent fixed line, see stage 8) has to be attached manually by the human before hitting send in their mail client.
