...
export function classifyQueueCategory(input: QueueCategoryInput): QueueCategoryKey {
  ...
  if (
    ...
    /\bathletics\b|\bseahawks\b|\bsounders\b|\bkraken\b|\bmariners\b|\bstorm\b|\brainiers\b|\breign fc\b|\blearfield\b|\bfan.?culture\b|\bgame-day\b|\bgameday\b|\bballpark\b/.test(
      text
    )
  ) {
    return "sports";
...
export function countQueueCategories<T extends QueueCategoryInput & { category?: string | null }>(
  items: T[]
  ...
  for (const item of items) {
    ...
    const key =
      stored && (QUEUE_CATEGORY_KEYS as readonly string[]).includes(stored)
        ? (stored as QueueCategoryKey)
